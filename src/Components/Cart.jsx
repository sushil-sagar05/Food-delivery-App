...
function Cart() {
  ...
  return (
    <>
    <div className={`fixed p-5 right-0 top-0 w-full sm:w-[20vw] h-full sm:h-full bg-white ${activeCart ? "translate-x-0" : "translate-x-full"} transition-all duration-500 z-50`}>
      ...
      {cartItems.length > 0 ? cartItems.map((food) => {
        ...
        <div className='absolute bottom-0 w-full sm:w-auto'>
            <h3 className='font-semibold text-gray-800'>Items: {totalqty} </h3>
            <h3 className='font-semibold text-gray-800'> Total Amount: {totalPrice} </h3>
            <hr className='w-full sm:w-[18vw] my-2' />
            <button className='bg-green-500 font-bold px-3 text-white py-2 rounded-lg w-full sm:w-[18vw] mb-5'> Checkout</button>
        </div>
        {cartItems.length > 0 ? <button 
           onClick={() => ClearCart()}
           className='bg-green-500 font-bold px-3 text-white py-2 my-3 rounded-lg w-full sm:w-[8vw] mb-5'>Clear Cart</button> : ""}
    </div>
    <FaShoppingCart 
    onClick={()=> setactiveCart(!activeCart)}
